...
def add_song(t, song, category):
    ...
    if root(t) == category:
        ...
        inner_branches = [add_song(branch, song, category) for branch in branches(t)]
        return tree(root(t), inner_branches)
...
def delete(t, target):
    ...
    if root(t) == target:
        return None
    elif is_leaf(t) and t == target:
        return None
    elif is_leaf(t):
        return t
    else:
        new_branches = list(filter(None, [delete(branch, target) for branch in branches(t)]))
